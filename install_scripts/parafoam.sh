...
# Package name PKG_NAME=OpenFOAM-v3.0+
# MPI  = $INSTALL_DIR/openmpi
# build dir=BUILD_DIR=$INSTALL_DIR/OpenFOAM
# logs in $INSTALL_DIR/$MY_PREFIX_DIR/package_logs 
# -----------------------------------------------------------------------------

echo 
echo " --------- Start Parafoam.sh script -------- "
echo
export SCRIPT_NAME=ParaFOAM
...
if [ ! -d "$INSTALL_DIR/OpenFOAM" ]; then
  ...
  echo $SCRIPT_NAME "Openfoam dir does  exist: no extraction tar"
fi
...
echo $SCRIPT_NAME " 2b : make paraview " log in $INSTALL_DIR/$MY_PREFIX_DIR/package_logs/parafoam_make.log
...
# mkdir -p $FOAM_RUN
#  
# echo $SCRIPT_NAME " in oreder to run set the environment with" 
