...
with cubic Hermite polynomials, as presented in Ref.~\citen{Intech2011} (default option).
...
{\sc multicompo} data structure where the interpolation is performed. This name must be set in the RHS of the  \dstr{NCR:} data structure.
...
If a {\sc map} object is defined on the RHS of structure \dstr{ncr\_data}, and if the \moc{TABLE} keyword is set, some information required to set the interpolation points is found in this object. In this case, the {\tt NCR:} operator search the {\sc multicompo} object for global or local parameters  having an arbitrary name specified in the {\sc map} object or set directly in  this module. Note that any parameter's value set directly in this module prevails on a value stored in the {\sc map} object.
