...
$[$ \moc{DEPL} $\{$ \moc{LIB:} $\{$ \moc{DRAGON} $|$ \moc{WIMS} $|$  \moc{WIMSD4}  $|$  \moc{WIMSAECL} $|$ \moc{NDAS}  $\}$ \moc{FIL:} \dusa{NAMEFIL} \\
\hskip 0.8cm $|$ \moc{LIB:} $\{$ \moc{APLIB2} $|$ \moc{APXSM} $\}$ \moc{FIL:} \dusa{NAMEFIL} \dstr{descdeplA2} \\
\hskip 0.8cm $|$ \dusa{ndepl} \dstr{descdepl} $\}$ $]$ \\
$[[$ \moc{MIXS} \moc{LIB:} \\
\hskip 0.8cm  $\{$ \moc{DRAGON} $|$ \moc{MATXS}  $|$ \moc{MATXS2}   $|$
                  \moc{WIMS}   $|$ \moc{WIMSD4} $|$ \moc{WIMSAECL} $|$ \moc{NDAS}  $|$    
                  \moc{APLIB1} $|$ \moc{APLIB2} $|$ \moc{APXSM} $\}$ \\
\hskip 0.8cm  \moc{FIL:} \dusa{NAMEFIL} 
$[[$ \dstr{descmix1} $]]$ $]]$
\end{DataStructure}
...
\moc{MIX} $[$ \dusa{matnum} $]$ $\{$ \\
\hskip 1.0cm $[$\dusa{temp} $[$ \dusa{denmix} $]~]~~[$ \moc{NOEV} $]$\\
\hskip 2.0cm $[[$  $[$ \dusa{NAMALI} \moc{=} $]$ \dusa{NAMISO} \dusa{dens} $[$ $\{$ \dusa{dil} 
    $|$ \moc{INF} $\}$ $]$\\
\hskip 2.0cm $[~[$ \moc{CORR} $]$ \dusa{inrs} $]~[$ \moc{DBYE} \dusa{tempd} $]$ $[$ \moc{SHIB} 
\dusa{NAMS} $]~[$ \moc{THER} \dusa{ntfg} \dusa{HINC} $[$ \moc{TCOH} \dusa{HCOH} $]~]$ \\
...
\moc{SHI:} operator (see \Sect{SHIData}). In this case, the dilution is only used
...
included with the cross sections when using a \moc{MATXS} or \moc{MATXS2} format
library.
...
The structure \dstr{descmix2} is used to describe the modifications in the
