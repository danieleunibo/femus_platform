...
$[~\{$ \moc{PRIM} $[$ \dusa{ielem} $[$ \dusa{isplh} $]~]~|$ \moc{DUAL} $[$ \dusa{ielem} \dusa{icol} $[$ \dusa{isplh} $]~]~|$ \moc{MCFD} $[$ \dusa{ielem} $[$ \dusa{isplh} $]~]~|$ \moc{LUMP} $[$ \dusa{ielem} $]~\}~]$ \\
...
1 (full hexagons), $>1$ for performing a mesh-splitting in $3 \times$\dusa{isplh}$^2$
