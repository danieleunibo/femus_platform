...
\item[\moc{SPLITH}] keyword to specify that a triangular mesh splitting level for \moc{HEXT}, \moc{HEXTCEL}, \moc{HEXTZ} and \moc{HEXTCELZ}type geometries is to be considered. This is valid only if \dusa{nhr}=1. 

\item[\dusa{isplth}] value of the triangular mesh splitting. Its use is similar to \dusa{nhr} except that each sector of the hexagonal cell will be filled by a unique mixture.
