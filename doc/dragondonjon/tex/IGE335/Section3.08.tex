...
~~~~~$[$ \dusa{REFGEO} $[$ \dusa{MACROGEO} $]~]~[$ \dusa{REFPIJ} $]$ \moc{::} \dstr{descedi}
\end{DataStructure}
...
\hskip 1.0cm \moc{CELL} $[~\{$ \moc{SYBIL} $|$ \moc{EXCELL} $|$ \moc{NXT} $|$ \moc{DEFAULT} $|$ \moc{UNFOLD} $\}~]~[$ \moc{REMIX} (\dusa{imixm2}(ii),ii=1,nbmix2) $]~|$ \\
\hskip 1.0cm \moc{REGI} (\dusa{iregm}(ii),ii=1,nregio) $|$ \\
\hskip 1.0cm \moc{MIX} $[$ (\dusa{imixm}(ii),ii=1,nbmix) $]~\}$ $]$ \\
$[$ \moc{TAKE} $\{$ \\
\hskip 1.0cm \moc{REGI} (\dusa{iregt}(ii),ii=1,nregio) $|$ \\
\hskip 1.0cm \moc{MIX} (\dusa{imixt}(ii),ii=1,nbmix) $\}$ $]$  \\
$[$ $\{$ \moc{P0W} $|$ \moc{P1W\_L} $|$ \moc{P1W\_T} $\}$ $]$ \\
...
\hskip 1.0cm $[$ \moc{REAC} \dusa{nreac} (\dusa{HREAC}(i),i=1,\dusa{nreac}) $]~]$\\
...
\item[\moc{REMIX}] the cell-by-cell homogenization produced by option \moc{CELL} is further homogenized according to \dusa{imixm2}
...
the cell-by-cell homogenization was performed. In the editing routines a value of \dusa{imixm2}=0 allows the corresponding
...
module \moc{ASM:} (see \Sect{descasm}).\cite{ALSB2}
...
is recovered by direct homogenization and condensation of the flux unknown components corresponding to external boundary and used with the current iteration method in Eurydice. This keyword is only compatible if a \moc{SYBILT:} tracking is used and if
