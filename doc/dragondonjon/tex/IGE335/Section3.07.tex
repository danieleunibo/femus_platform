...
\dusa{FLUNAM} \moc{:=} \moc{FLU:} $[$ \dusa{FLUNAM} $]$ \dusa{PIJNAM} 
\dusa{LIBNAM}  \dusa{TRKNAM} $[$ \dusa{TRKFIL} $]$ \\
$~~~~[$ \dusa{TRKFLP} \dusa{TRKGPT} $]$ \moc{::} \dstr{descflu}
\end{DataStructure}
...
\item[\dusa{TRKGPT}] {\tt character*12} name of the \dds{gpt} data structure
...
The \dstr{descleak} structure allows the following information to be specified:

\begin{DataStructure}{Structure \dstr{descleak}}
$\{$ \moc{LKRD} $|$ \moc{RHS} $|$\moc{P0} $|$ \moc{P1} $|$ \moc{B0} $|$ \moc{B1} $|$ 
\moc{B0TR} $\}$ \\
