...
\section {The ANSI C CLE-2000 API}
...
typedef struct LIFO_NODE {     /* node in last-in-first-out (lifo) stack */
   ...
   union {
       ...
       lcm *lcm;               /* handle towards a LCM object */
...
\hline
{\tt value.lcm} & {\it lcm*} & LCM object (memory-resident). This data is given or is available at output if and only if {\tt type} $= 3$. The LCM object is closed.\\
\hline
...
This technical report contains the precise description of each ANSI~C function available in the LCM API and dedicated
...
\hline
{\tt namp} & {\it char[13]} & name of the LCM object if {\tt imp=0}. \\
\hline
...
elements in a daughter heterogeneous list or the number of consecutive 
32-bit memory words required to store an array of elementary type. As an example, the length required
to store an array of 64-bit real numbers is twice its dimension.
...
\multicolumn{3}{|c|}{\bf output parameters:} \\
\hline
{\tt ilong} & {\it int\_32*} & length of the block in 32-bit words. =$-1$ for a daughter associative table; 
...
\multicolumn{3}{|c|}{\bf output parameters:} \\
\hline
{\tt namlcm} & {\it char*} & name of the LCM object. \\
\hline
{\tt nammy} & {\it char*} & name of the associative table at address {\tt iplist}. $=${\tt '/'} if the associative table
...
\multicolumn{3}{|c|}{\bf output parameters:} \\
\hline
{\tt ilong} & {\it int\_32*} & length of the block in 32-bit words. =0 if the block does't exist. \\
\hline
{\tt itylcm} & {\it int\_32*} & type of information. =0 associative table; =1 32-bit integer; 
                                    =2 32-bit real; =3 {\tt character*4} data; 
				    =4 64-bit real; =5 32-bit logical; =6 64-bit complex; =10 heterogeneous list; 
...
{\tt namp} & {\it char*} & name of the block. \\
\hline
{\tt ilong} & {\it int\_32} & length of the block in 32-bit words. If the array contains $N$ real\_64 values, 
...
\hline
{\tt data} & {\it int\_32*} & array of dimension $\ge$ {\tt ilong} to be copied into the LCM object. 
                                   Array elements {\tt data[0]} to {\tt data[ilong-1]}
...
{\tt iofset = setara\_c(ilong)} or {\tt iofset = (int\_32*)malloc(ilong*sizeof(int\_32))}.
...
{\tt namp} & {\it char*} & name of the block. \\
\hline
{\tt ilong} & {\it int\_32} & length of the block in 32-bit words. \\
\hline
{\tt itylcm} & {\it int\_32} & type of information. =1 32-bit integer; 
                                    =2 32-bit real; =3 {\tt character*4} data; 
				    =4 64-bit real; =5 32-bit logical; =6 64-bit complex; =99 undefined. \\
\hline
{\tt iofset} & {\it int\_32*} & address of the data array, as returned by {\tt setara\_c}. Data elements
                                     {\tt iofset[0]} to {\tt iofset[ilong-1]} 
...
than LCM, and where the block must {\sl not} be deallocated by the LCM API. In this case, it is imperative
...
\hline
{\tt data} & {\it int\_32*} & array of dimension $\ge$ {\tt ilong} to be copied into the LCM object. 
                                   Array elements {\tt iofset[0]} to {\tt iofset[ilong-1]}
...
However, {\tt lcmsix\_c} is kept available in the LCM API for historical reasons.
...
\section {The ISO Fortran LCM API}

The ISO Fortran LCM API is a set of Fortran-2003 wrapper subroutines or functions programmed around the ANSI-C functions of the LCM API. This implementation is using
...
Any subroutines or functions using the Fortran LCM API must include a {\tt USE} statement of the form
\begin{verbatim}
USE GANLIB
\end{verbatim}
...
\hline
{\tt NAMP} & {\it CHARACTER*12} & name of the LCM object if  {\tt IMP=0}. \\
\hline
...
\multicolumn{3}{|c|}{\bf output parameters:} \\
\hline
{\tt ILONG} & {\it INTEGER} & length of the block in 32-bit words. =$-1$ for a daughter associative table; 
                                    =$N$ for a daughter heterogeneous list containing $N$ components; 
		                 =0 if the block does't exist.\\
\hline
...
\multicolumn{3}{|c|}{\bf output parameters:} \\
\hline
{\tt NAMLCM} & {\it CHARACTER*12} & name of the LCM object. \\
\hline
...
\multicolumn{3}{|c|}{\bf output parameters:} \\
\hline
{\tt ILONG} & {\it INTEGER} & length of the block in 32-bit words. =0 if the block does't exist. \\
\hline
{\tt ITYLCM} & {\it INTEGER} & type of information. =0 associative table; =1 32-bit integer; 
                                    =2 32-bit real; =3 {\tt character*4} data; 
				    =4 64-bit real; =5 32-bit logical; =6 64-bit complex; =10 heterogeneous list; 
...
{\tt LCMGDL}) or {\sl without} copy ({\tt LCMPPD}, {\tt LCMGPD}, {\tt LCMPPL} or
{\tt LCMGPL}).
...
{\tt NAMP} & {\it CHARACTER*12} & name of the block. \\
\hline
{\tt ILONG} & {\it INTEGER} & length of the block in 32-bit words. If the array contains $N$ real\_64 values, 
                                   {\tt ilong} must be set to $2 \times N$. \\
\hline
...
{\tt DATA} & {\it CLASS(*)} & array of dimension $\ge$ {\tt ILONG} to be copied into the LCM object. 
                                   Array elements {\tt DATA(1)} to {\tt DATA(ILONG)} must be initialized before the call to {\tt LCMPUT}. \\
\hline
\end{tabular}
...
Subroutine {\tt LCMGPD} implements direct {\sl pinning} on LCM data structures. It represents an advanced capability of the LCM API and should only be used in situations where
...
Subroutine {\tt LCMPPD} implements direct {\sl pinning} on LCM data structures. It represents an advanced capability of the LCM API and should only be used in situations where
...
\noindent so that he useful information is accessed in a Fortran variable {\tt IDATA} set using a {\tt C\_F\_POINTER} function:
\begin{verbatim}
   USE GANLIB
   ...
   TYPE(C_PTR) :: IOFSET
...
{\tt NAMP} & {\it CHARACTER*12} & name of the block. \\
\hline
{\tt ILONG} & {\it INTEGER} & length of the block in 32-bit words. \\
\hline
{\tt ITYLCM} & {\it INTEGER} & type of information. =1 32-bit integer; 
                                    =2 32-bit real; =3 {\tt character*4} data; 
				    =4 64-bit real; =5 32-bit logical; =6 64-bit complex; =99 undefined.\\
...
\hline
{\tt DATA} & {\it CLASS(*)} & array of dimension $\ge$ {\tt ILONG} to be copied into the LCM object. 
                                   Array elements {\tt IOFSET(1)} to {\tt IOFSET(ILONG)}
                                   must be initialized before the call to {\tt LCMPDL}. \\
\hline
\end{tabular}
...
Subroutine {\tt LCMGPL} implements direct {\sl pinning} on LCM data structures. It represents an advanced capability of the LCM API and should only be used in situations where
the economy of computer resources is a critical issue. The {\tt C\_PTR} address is the ANSI~C pointer of a block of
information made available into a Fortran program. If {\tt IOFSET} is a {\tt C\_PTR} address, the useful information is
accessed in a Fortran variable {\tt IDATA} set using
\begin{verbatim}
...
Subroutine {\tt LCMPPL} implements direct {\sl pinning} on LCM data structures. It represents an advanced capability of the LCM API and should only be used in situations where
the economy of computer resources is a critical issue. The memory block stored by {\tt LCMPPL} must be previously
allocated by a call to {\tt LCMARA} of the form
\begin{verbatim}
IOFSET=LCMARA(ILONG)
\end{verbatim}
...
However, {\tt LCMSIX} is kept available in the LCM API for historical reasons.
...
\multicolumn{3}{|c|}{\bf output parameter:} \\
\hline
{\tt IFILE} & {\it TYPE(C\_PTR)}  & address of the allocated file. Equal to {\tt C\_NULL\_PTR} if the allocation failed.  \\
\hline
\end{tabular}
...
\multicolumn{3}{|c|}{\bf input parameters:} \\
\hline
{\tt FILE\_PT} & {\it TYPE(C\_PTR)} &  address of the allocated file (as returned by {\tt FILOPN}). \\
\hline
{\tt IACTIO} & {\it INTEGER}  & action on file.
$=1$: to keep the file;
$=2$: to delete the file.  \\
